...
namespace Mountains
{
    ...
    public class TweenTransformAction : TweenActionBase
    {
        ...
        [Tooltip("지정하면 이 오브젝트의 현재 값을 목표로 삼는다. 비워두면 아래 endValue를 쓴다.")]
        [GizmoTarget("목표")] public Transform destination;
        [Tooltip("destination이 비어 있을 때 쓰는 목표 값.")]
        public Vector3 endValue;

        [Header("옵션")]
        [Tooltip("켜두면 목표 값을 현재 값에 더한다. LookAt에는 적용되지 않는다.")]
        ...
        // LocalMove에 월드 좌표를 그대로 넣으면 부모가 움직인 만큼 엉뚱한 곳으로 가므로
        // 부모 기준으로 변환해준다.
