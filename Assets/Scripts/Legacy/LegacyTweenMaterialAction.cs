...
namespace Mountains
{
    ...
    //
    // 반드시 renderer.material(런타임 인스턴스)만 건드리고 sharedMaterial은 절대 만지지
    // 않는다 — sharedMaterial은 에셋 그 자체라, 에디터에서 값을 쓰면 .mat 파일이 dirty로
    // 표시돼 저장/빌드 때 디스크에 기록된다(전에 WindMultiplier가 PT 머티리얼 11개를
    // 건드려 커밋에 잡혔던 것과 같은 문제). renderer.material은 접근하는 순간 이 렌더러
    // 전용 복제본이 생기므로 원본 에셋은 그대로다.
    // 새로 붙이지 못하게 Add Component 메뉴에서 감춘다 — 이미 붙어 있는
    // 컴포넌트는 그대로 동작하므로 마이그레이션에는 영향이 없다.
    [AddComponentMenu("")]
    public class LegacyTweenMaterialAction : LegacyTweenActionBase
    {
        ...
        [Header("대상 렌더러")]
        [Tooltip("비워두면 target(비어 있으면 이 오브젝트) 아래에서 Renderer를 찾아 쓴다.")]
        public Renderer[] renderers = new Renderer[0];
        [Tooltip("renderers가 비어 있을 때, 자식의 Renderer까지 포함할지.")]
        public bool includeChildren = true;
        [Tooltip("-1이면 렌더러가 가진 머티리얼 전부, 0 이상이면 그 슬롯 하나만.")]
        ...
        [Tooltip("Emission은 1을 넘는 HDR 값을 줘야 눈에 띄게 빛난다(색 피커의 Intensity).")]
        ...
        [Tooltip("비워두지 않으면 값을 바꾸기 전에 이 셰이더 키워드를 켠다. URP Lit은 머티리얼에서 " +
            "Emission 체크를 꺼두면 _EMISSION 키워드가 없어서 _EmissionColor를 바꿔도 아무 " +
            "변화가 없다 — 그럴 때 _EMISSION을 넣어준다.")]
        public string enableKeyword = "";

        // 여러 머티리얼을 한 Sequence로 묶으므로 ease는 개별 트윈에 직접 건다 — 시퀀스에
        // 걸면 타임라인 진행까지 휘어서 두 번 먹는다.
        ...
        protected override Tween CreateTween(Transform target)
        {
            ...
            {
                Debug.LogWarning($"[LegacyTweenMaterialAction] {name}: 대상 Renderer를 찾지 못했습니다.");
                return null;
            }
            ...
            foreach (var material in _materials)
            {
                ...
                {
                    Debug.LogWarning($"[LegacyTweenMaterialAction] {material.name}에 '{propertyName}' " +
                        "프로퍼티가 없어 건너뜁니다.");
                    continue;
                }
        ...
        // renderer.material / renderer.materials는 접근하는 순간 이 렌더러 전용 복제본을
        // 만든다 — 그래서 여기서 모은 머티리얼을 바꿔도 다른 오브젝트나 에셋에 번지지 않는다.
