...
namespace Mountains
{
    ...
    // 작아져 사라진다. 코인이 런타임에 생성되므로 범용 Tween 액션(LegacyTweenJumpAction 등)으로는
    ...
    [AddComponentMenu("")]
    public class LegacyCoinBurstAction : LegacyTriggerAction
    {
        [Header("코인")]
        ...
        public override void Execute(Action onComplete)
        {
            if (coinPrefab == null)
            {
                Debug.LogWarning("[LegacyCoinBurstAction] coinPrefab이 비어 있어 연출을 건너뜁니다.");
                onComplete?.Invoke();
                return;
            }
