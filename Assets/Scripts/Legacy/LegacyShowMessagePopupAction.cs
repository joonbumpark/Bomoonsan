...
namespace Mountains
{
    // EventTrigger 스텝에서 메시지 팝업을 띄우는 액션. 기본값(waitForClose)은 팝업이 닫힐
    // 때까지 다음 스텝으로 넘어가지 않는다 — 안내를 읽기 전에 연출이 진행되면 의미가 없다.
    // 새로 붙이지 못하게 Add Component 메뉴에서 감춘다 — 이미 붙어 있는
    // 컴포넌트는 그대로 동작하므로 마이그레이션에는 영향이 없다.
    [AddComponentMenu("")]
    public class LegacyShowMessagePopupAction : LegacyTriggerAction
    {
        ...
        [Tooltip("켜두면 팝업이 닫힐 때까지 다음 스텝으로 넘어가지 않는다. 꺼두면 팝업을 띄우고 " +
            "곧바로 다음 스텝이 진행된다.")]
        ...
        public override void Execute(Action onComplete)
        {
            ...
            if (waitForClose)
            {
                request.onClosed = () => onComplete?.Invoke();
