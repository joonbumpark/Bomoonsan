using System;
using System.Collections;
using UnityEngine;

namespace Mountains
{
    // EventTrigger 스텝에서 토스트를 띄우는 액션. 토스트는 흐름을 끊지 않는 알림이라
    // 기본은 곧바로 완료 처리하고, 연출 순서상 다 보여준 뒤 넘어가야 할 때만 기다린다.
    // 새로 붙이지 못하게 Add Component 메뉴에서 감춘다 — 이미 붙어 있는
    // 컴포넌트는 그대로 동작하므로 마이그레이션에는 영향이 없다.
    [AddComponentMenu("")]
    public class LegacyShowToastAction : LegacyTriggerAction
    {
        [TextArea(1, 4)] public string message;
        [Tooltip("0 이하면 ToastUI의 기본 표시 시간을 쓴다.")]
        public float duration = 0f;
        [Tooltip("켜두면 표시 시간이 끝날 때까지 다음 스텝으로 넘어가지 않는다.")]
        public bool waitForDuration;

        public override void Execute(Action onComplete)
        {
            Toast.Show(message, duration);

            if (!waitForDuration)
            {
                onComplete?.Invoke();
                return;
            }

            StartCoroutine(CompleteAfterDuration(onComplete));
        }

        IEnumerator CompleteAfterDuration(Action onComplete)
        {
            float wait = duration > 0f ? duration : ToastUI.GetOrCreate().defaultDuration;
            yield return new WaitForSeconds(wait);
            onComplete?.Invoke();
        }
    }
}
